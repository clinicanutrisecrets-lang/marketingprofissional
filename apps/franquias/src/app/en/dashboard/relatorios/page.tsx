...
export default async function RelatoriosPageEN() {
  ...
  const list = (relatorios ?? []) as Array<Record<string, unknown>>;
    ...
    <main className="min-h-screen bg-brand-muted">
      <div className="mx-auto max-w-5xl p-6 lg:p-8">
        ...
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-brand-text">Weekly reports</h1>
          <p className="text-sm text-brand-text/60">
            Performance of your Instagram posts and Facebook Page engagement,
            fetched via the Graph API (instagram_manage_insights,
            pages_read_engagement) plus AI analysis.
          </p>
        </header>
        ...
        {list.length === 0 ? (
          <div className="rounded-2xl bg-white p-8 text-center shadow-sm">
            ...
            <p className="text-sm text-brand-text/70">
              No reports yet. Once your posts start being published, they will
              appear here every Friday.
            </p>
          </div>
        ) : (
                ...
                <header className="mb-4 flex items-center justify-between">
                  <div>
                    <h2 className="text-lg font-semibold">
                      Week of {new Date(r.semana_inicio as string).toLocaleDateString("en-US")}
                    </h2>
                    <p className="text-xs text-brand-text/60">
                      to {new Date(r.semana_fim as string).toLocaleDateString("en-US")}
                    </p>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-xs text-brand-text/60">engagement</div>
                  </div>
                </header>

                <div className="mb-4 grid grid-cols-3 gap-4">
                  <Metric label="Posts" value={(r.total_posts as number) ?? 0} />
                  <Metric label="Reach" value={formatK((r.total_alcance as number) ?? 0)} />
                  <Metric label="Interactions" value={formatK((r.total_engajamento as number) ?? 0)} />
                </div>
