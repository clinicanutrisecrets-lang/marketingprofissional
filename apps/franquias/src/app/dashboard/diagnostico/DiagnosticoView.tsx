...
export function DiagnosticoView({ franqueadaId, diagnosticoInicial }: Props) {
  ...
  async function gerarNovo() {
    ...
    try {
      ...
      // recarrega pra buscar o recém-criado
    ...
    return (
      <div className="mt-8 rounded-2xl bg-white p-8 text-center shadow-sm">
        <h2 className="text-lg font-semibold text-brand-text">
          Você ainda não tem diagnóstico
        </h2>
        <p className="mt-2 text-sm text-brand-text/70">
          Em ~30 segundos nosso agente analisa seu perfil e entrega 10 mudanças
        ...
        >
          {loading ? "Analisando seu perfil…" : "Gerar meu diagnóstico"}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="mt-8 space-y-6">
      ...
      {/* MUDANÇAS PRIORIZADAS — destaque */}
      <section className="rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-brand-text">10 mudanças priorizadas</h2>
        ...
        <ol className="mt-5 space-y-4">
          {diagnostico.mudancas_priorizadas.map((m) => (
            <li
              key={m.ordem}
              className="flex gap-4 rounded-lg border border-brand-text/10 p-4"
            >
              ...
              <div className="flex-1">
                <div className="flex items-baseline gap-3">
                  <h3 className="font-semibold text-brand-text">{m.titulo}</h3>
                  <span className="rounded-full bg-brand-muted px-2 py-0.5 text-[10px] uppercase tracking-wide text-brand-text/70">
                    {m.onde_aplicar}
                  </span>
                </div>
                <p className="mt-1 text-sm text-brand-text/80">{m.descricao}</p>
                <div className="mt-2 flex gap-4 text-[11px] text-brand-text/60">
                  <span>
                    Impacto:{" "}
                    <strong
                      className={
      ...
      <section className="rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-brand-text">Como seu perfil é lido</h2>
        <div className="mt-4 space-y-4 text-sm text-brand-text/85 leading-relaxed">
          <div>
            <div className="text-xs uppercase tracking-wider text-brand-text/50 mb-1">
              Primeira impressão
            </div>
          ...
          <div>
            <div className="text-xs uppercase tracking-wider text-brand-text/50 mb-1">
              Linha editorial que seu perfil conta hoje
            </div>
      ...
      {/* FORÇAS / FRAQUEZAS / GARGALOS em grid */}
      <div className="grid gap-6 md:grid-cols-2">
        <Bloco titulo="Forças" itens={diagnostico.forcas} cor="green" />
        <Bloco titulo="Fraquezas" itens={diagnostico.fraquezas} cor="red" />
        <Bloco
          titulo="Gargalos de crescimento"
          itens={diagnostico.gargalos_crescimento}
          cor="amber"
        />
        <Bloco
          titulo="Gargalos de conversão (virar lead)"
          itens={diagnostico.gargalos_conversao}
          cor="amber"
        />
        <Bloco
          titulo="Erros de percepção"
          itens={diagnostico.erros_percepcao}
          cor="red"
        />
        <Bloco
          titulo="Oportunidades rápidas (quick wins)"
          itens={diagnostico.oportunidades_rapidas}
          cor="green"
        />
      </div>
      ...
      {/* SUGESTÕES */}
      <section className="rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-brand-text">Sugestões acionáveis</h2>

        <div className="mt-5 space-y-5">
          <div>
            <h3 className="text-sm font-semibold text-brand-text/80 mb-2">
              Novas versões de bio
            </h3>
            <ul className="space-y-2">
              {diagnostico.sugestoes_bio.map((b, i) => (
                <li
                  key={i}
                  className="rounded-lg border border-brand-text/10 bg-brand-muted/30 p-3 text-sm"
                >
                  {b}
                </li>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {diagnostico.sugestoes_destaques.map((d, i) => (
                <span
                  key={i}
                  className="rounded-full border border-brand-text/10 bg-white px-3 py-1 text-sm"
                >
                  {d}
                </span>
              ))}
          ...
          <div>
            ...
            <p className="rounded-lg bg-brand-muted/30 p-3 text-sm text-brand-text/85">
          ...
          <div>
            <h3 className="text-sm font-semibold text-brand-text/80 mb-2">
              Ideias de reposicionamento
            </h3>
            <ul className="list-disc space-y-1 pl-5 text-sm text-brand-text/85">
