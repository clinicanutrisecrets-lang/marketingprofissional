...
function buildSystemBase(perfilSlug: string): string {
  const icp = icpParaPerfil(perfilSlug);
  const ctx = perfilContexto(perfilSlug);
  return `
Você é estrategista sênior de marketing direto para nicho de saúde premium (nutrição clínica, medicina funcional, nutrigenética). Domina copywriting, posicionamento, funil orgânico e criação de mecanismos únicos de oferta.

PERFIL TARGET: ${ctx === "B2B_nutris" ? "@scannerdasaude (B2B — nutricionistas)" : "@nutrisecrets (B2C — paciente final premium)"}

${icp}

Regras duras:
- Compliance CFN: zero promessa de cura, zero antes/depois com prazo, zero linguagem milagreira.
- Estratégia brutalmente honesta, específica, defensável em conteúdo e em anúncio.
- Sem fórmulas genéricas de infoproduto hustle — o público premium de saúde lê isso e foge.
...
Output JSON:
{
  ...
  "inimigo_comum": "o que o público e a marca combatem juntos",
...
export type InputPlanejamento = {
  perfilSlug: string;
  tipo: TipoPlanejamento;
  input: Record<string, unknown>;
};

export type ResultPlanejamento = {
  ok: boolean;
...
export async function executarPlanejamento(
  params: InputPlanejamento,
): Promise<ResultPlanejamento> {
  const aline = createAlineClient();

  const { data: p } = await aline
    .from("perfis")
    .select("id, slug, nome, objetivo, tom, pilares, regras_especiais")
  ...
  const systemText = `${buildSystemBase(params.perfilSlug)}\n\n=== CONTEXTO PERFIL ===\n${JSON.stringify(perfilRow, null, 2)}\n\n=== TIPO: ${params.tipo} ===\n${SCHEMAS[params.tipo]}`;
  ...
  return { ok: true, planejamentoId: (salvo as { id: string }).id, output: parsed };
}
